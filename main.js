...
let monologue = [
  ...
  "ANGRY!!!",
  "No amount of nature walking could tame him...",
  "His rage weathered even the snuggliest blanket...",
...
let creditsText = [
  "Created by D.A. Marcyes",
  "for the April Fools CodeJam",
  "Music composed by",
  "D.A. Marcyes",
  ...
  "a frustrated D.A. Marcyes",
  "",
  "First place awarded to",
  "D.A. Marcyes",
  "In memorium of all the hours",
  "spent putting this together",
  "Special thanks to Rick Astley",
  "",
];
...
let quizAnswers = [
  ...
  ["God", "April Fools Gods", "Himself", "You"],
...
function quizLoop() {
  ...
  for (var answer = 0; answer < quizAnswers[quizIt].length; answer++) {
    ...
    ctx.fillText(
      quizAnswers[quizIt][answer],
      maxWidth / 2,
      answerY[answer] + maxHeight / 25
    );
...
const Adam = {
  ...
  get juice() {
    return this._jump_juice;
  },
  set juice(juice_quantity) {
    this._jump_juice = juice_quantity;
  },
  ...
  drawAdam: function () {
    ...
    ctx.arc(
      this._x + adamWidth - eyeball_size,
      this._y + eyeball_size,
      eyeball_size,
      0,
      Math.PI * 2
    );
...
function gameLoop() {
  if (game) {
    ...
    moveAdam();
    //check to see if Adam has reached the goal
    if (Adam.x > maxWidth * 0.65 && Adam.y > maxHeight * 0.5) {
      game = false;
      credits = true;
      creditsLoop();
    }
...
let tempCounter = 0;
let letterToType = 0;
let creditsSpeed = 60; //ms
let line = 1;
let lineY = 0.25; //% of screen height
let astley = false;
async function creditsLoop() {
  //set line height to top if on new chunk of text
  if (line % 2 != 0) {
    lineY = 0.25;
  }
  //draw background
  ...
  //draw clouds
  cloudLoop();
  //draw semi-opaque overlay for readability
  ...
  ctx.shadowColor = "rgba(0, 0, 255, 0.5)";
  ctx.font = "bold 1.5em verdana, sans-serif";
  //add line break if on second line
  if (line % 2 == 0) {
    lineY = 0.25;
    ctx.fillText(creditsText[tempCounter - 1], maxWidth / 2, maxHeight * lineY);
    lineY = 0.4;
  }

  tempText += creditsText[tempCounter].charAt(letterToType);
  //print each letter of the end credits so long as they shall exist
  ctx.fillText(tempText, maxWidth / 2, maxHeight * lineY);
  if (tempCounter == creditsText.length - 1) {
    astley = true;
    ctx.fillText("Click for Secret Ending", maxWidth / 2, maxHeight * lineY);
  } else if (letterToType <= creditsText[tempCounter].length) {
    letterToType += 1;
    setTimeout(creditsLoop, creditsSpeed + Math.random() * 5 - 1);
  } else if (letterToType > creditsText[tempCounter].length) {
    letterToType = 0;
    tempCounter += 1;
    tempText = "";
    line += 1;
    //pause for readability if beginning a new text chunk
    if (line % 2 != 0) {
      await sleep(500);
    }
    await creditsLoop();
...
let typingSpeed = 500;
function printText() {
  //go to quiz once monologue is done, else keep cycling through
  if (monologueIt == monologue.length) {
    quiz = true;
    cutScene = false;
    quizLoop();
  } else if (cutScene) {
    ...
    if (letter <= monologue[monologueIt].length) {
      ...
      //print continue prompt after finished typing
      ctx.fillStyle = "rgba(256,256,256,0.5)";
  ...
  if (randomChance <= cloudSpawnRate) {
    ...
    clouds.push(
      (Cloud = {
        ...
        y: yCoord,
        line1: function () {
          return this.x + firstX;
        },
        line2: function () {
          return this.y - maxHeight * 0.05;
        },
        arc1: firstRadius,
        arc2: secondRadius,
        arcX: function () {
          return this.line1() + this.arc1 + this.arc2;
        },
        setX: function (x) {
          this.x = x;
        },
...
function clickHandler(x, y) {
  ...
  //handles clicks for cutscene
    ...
    if (!typing) {
      //click to continue after typing is done
      monologueIt += 1;
      cutSceneLoop();
    } else {
      //if click during typing, speed up typing
      clickCounter += 1;
      typingSpeed /= 2;
    }
    if (clickCounter == 3) {
      //if speed increased too much, skip cutscene
      cutScene = false;
      quiz = true;
  ...
  //handles clicks for end credits
  else if (astley) {
    let song = document.getElementById("song");
    song.pause();
...
async function moveRight(momentum) {
  ...
  if (momentum == "acc") {
    ...
    //if keyup, decelerate right unless right key is pressed
...
async function moveLeft(momentum) {
  ...
  if (momentum == "acc") {
    ...
    //if keyup, decelerate left unless right key is pressed
